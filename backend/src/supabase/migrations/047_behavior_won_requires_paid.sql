-- Fix the "bid history lie": a bid is only a WON booking when it's ACCEPTED
-- AND its payment was CAPTURED. Previously these analytics counted any
-- ACCEPTED bid as a booking/win/conversion, so a traveler whose card declined
-- (accepted-but-unpaid) showed as "won / 100% conversion". We're MoR: no
-- captured payment = no booking. Replaces the two functions from migration 038.
...
BEGIN
  SELECT
    COUNT(*),
    -- WON = accepted AND paid (payment captured), not merely accepted.
    COUNT(*) FILTER (WHERE b.status = 'ACCEPTED' AND pay.status = 'CAPTURED'),
    COUNT(*) FILTER (WHERE b.status = 'REJECTED'),
    COUNT(*) FILTER (WHERE b.status = 'CANCELLED'),
    ROUND(AVG(b.bid_per_night), 2),
    MIN(b.bid_per_night),
    MAX(b.bid_per_night),
    ROUND(AVG((p."retailPrice"::numeric - b.bid_per_night) / NULLIF(p."retailPrice"::numeric, 0) * 100)
          FILTER (WHERE p."retailPrice"::numeric > 0), 1),
  ...
  LEFT JOIN public.payments pay ON pay.bid_id = b.id
...
BEGIN
  ...
  -- Bookers = travelers with at least one ACCEPTED + CAPTURED bid.
  SELECT COUNT(DISTINCT b.student_id) INTO v_bookers
  FROM public.bids b
  JOIN public.payments pay ON pay.bid_id = b.id
  WHERE b.status = 'ACCEPTED' AND pay.status = 'CAPTURED';
